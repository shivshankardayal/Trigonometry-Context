...
label("$I$", i, align=SW, blue);
dot(i);
pair i1 = excenter(opposite(t.VA));
dot(i1);
label("$I_1$", i1, align=SW, blue);
pair i2 = excenter(opposite(t.VB));
dot(i2);
label("$I_2$", i2, align=SE, blue);
pair i3 = excenter(opposite(t.VC));
dot(i3);
label("$I_3$", i3, align=SW, 0.6*blue);
...
point M = relpoint(line(t.VA, t.VC), 1);
draw(point(t.VC)-- M);
label("$M$", M, align=SW, blue);
